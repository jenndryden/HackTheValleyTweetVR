...
public class DataVisualizer : MonoBehaviour {
    ...
    List<Color> meshColors)
    {
        ...
        Vector3 pos;
        pos.x = 0.25f * Mathf.Cos((lng) * Mathf.Deg2Rad) * Mathf.Cos(lat * Mathf.Deg2Rad);
        pos.y = 0.25f * Mathf.Sin(lat * Mathf.Deg2Rad);
        pos.z = 0.25f * Mathf.Sin((lng) * Mathf.Deg2Rad) * Mathf.Cos(lat * Mathf.Deg2Rad);
        p.transform.parent = Earth.transform;
        p.transform.position = pos;
        p.transform.localScale = new Vector3(1, 1, Mathf.Max(0.25f, value * ValueScaleMultiplier));
